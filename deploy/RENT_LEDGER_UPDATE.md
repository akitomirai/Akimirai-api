...
```text
https://miraiapi.cn/rent-ledger/latest.json
```

清单示例在 `deploy/rent-ledger-updates/latest.json`。`apkUrl` 可以写相对路径，例如 `rent-ledger-app.apk`，App 会解析为：

```text
https://miraiapi.cn/rent-ledger/rent-ledger-app.apk
```

## Nginx

当前服务器如果由 Nginx 接入 `miraiapi.cn`，在 HTTPS `server` 块里给反向代理 `location /` 前面增加：
...
这样 `https://miraiapi.cn/rent-ledger/latest.json` 会读取 `/var/www/rent-ledger/latest.json`。

## Caddy

参考 `deploy/rent-ledger-update.Caddyfile.example`，给 `miraiapi.cn` 增加静态目录：
...
```bash
curl https://miraiapi.cn/rent-ledger/latest.json
```
