export default {
    ...
    usage: {
      ...
      diagnostics: {
        ...
        retryOnly: '仅重试请求',
        minTotalMs: '总耗时下限(ms)',
        minFirstTokenMs: '首字下限(ms)',
        minFirstByteMs: '首包下限(ms)',
        requestStartedAt: '请求开始',
        recordedAt: '记录完成',
        totalDuration: '请求处理总耗时',
        ...
        bodyRead: '读取请求体',
        bodyReadDetail: '读取客户端请求体本身的耗时；不是距请求开始的累计时间',
        routing: '路由选择',
        routingDetail: '从进入路由阶段到最终上游转发前的累计耗时；包含并发等待、计费复检与重试影响',
        requestWritten: '上游请求写入完成',
        requestWrittenDetail: '最终上游尝试中，HTTP 客户端完成请求写入时距请求开始的累计时间；不代表上游已收到',
        firstByte: '上游响应头首字节',
        firstByteDetail: '最终上游尝试中，响应头首字节可用时距请求开始的累计时间；不是响应体或模型 Token',
        firstToken: '首个语义 Token',
        firstTokenDetail: '上游转发记录的首 Token 指标换算为距请求开始的累计时间；不同协议判定点可能不同',
        completed: '请求完成',
        completedDetail: '从处理开始到上游转发与响应处理完成后生成诊断快照的累计时间；不含异步用量落库',
        bodyBytes: '请求体大小',
        attempts: '重试与换号记录',
        startedOffset: '开始',
        writtenOffset: '写完',
