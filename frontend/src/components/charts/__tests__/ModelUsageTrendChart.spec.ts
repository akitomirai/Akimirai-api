...
describe('ModelUsageTrendChart', () => {
  ...
  it('builds cost datasets in canonical model order and fills missing buckets', () => {
    const wrapper = mount(ModelUsageTrendChart, {
      props: { trendData, metric: 'actual_cost' },
  ...
  it('uses request counts for the call trend metric', () => {
    const wrapper = mount(ModelUsageTrendChart, {
      props: { trendData, metric: 'requests' },
      global: { stubs: { LoadingSpinner: true } },
    })

    const chartData = JSON.parse(wrapper.find('.chart-data').text())
    expect(chartData.datasets[0].data).toEqual([8, 4])
    expect(wrapper.text()).toContain('Call trend')
  })
  ...
  it('keeps model labels and colors stable across metrics with Other last', () => {
    ...
    const costWrapper = mount(ModelUsageTrendChart, {
      props: { trendData: rankedByCost, metric: 'actual_cost' },
      global: { stubs: { LoadingSpinner: true } },
    })
    const requestWrapper = mount(ModelUsageTrendChart, {
      props: { trendData: rankedByCost, metric: 'requests' },
      global: { stubs: { LoadingSpinner: true } },
    })

    const costDatasets = JSON.parse(costWrapper.find('.chart-data').text()).datasets
    const requestDatasets = JSON.parse(requestWrapper.find('.chart-data').text()).datasets
    ...
    expect(requestDatasets.map(projectIdentity)).toEqual(costDatasets.map(projectIdentity))
  })
  ...
  it('renders loading and empty states without a chart', () => {
    const loading = mount(ModelUsageTrendChart, {
      props: { trendData: [], metric: 'requests', loading: true },
      global: { stubs: { LoadingSpinner: true } },
    })
