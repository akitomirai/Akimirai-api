...
describe('AccountUsageCell', () => {
  ...
  it.each([
    ...
    { tokens: 1_000_000, expected: 50, labels: ['1.0M', '100.00w'] },
    { tokens: 2_000_000, expected: 100, labels: ['2.0M', '200.00w'] },
    { tokens: 2_200_000, expected: 100, labels: ['2.2M', '220.00w'] }
  ])('Grok Free derives its 2M quota from local tokens: $tokens -> $expected%', async ({ tokens, expected, labels }) => {
    getUsage.mockResolvedValue({
      grok_billing: {
        period_type: 'weekly',
        usage_percent: null,
  ...
  it('Grok Free uses rolling 24h usage instead of today-only usage', async () => {
    ...
    expect(['1.5M', '150.00w'].some((label) => wrapper.text().includes(label))).toBe(true)
