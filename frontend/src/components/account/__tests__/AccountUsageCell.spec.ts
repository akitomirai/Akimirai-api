...
describe('AccountUsageCell', () => {
  ...
  it('Key 账号会展示 today stats 徽章并带 A/U 提示', async () => {
		...
		expect(wrapper.text()).toContain('100000.00w')
  ...
  it.each([
    { tokens: 0, expected: 0, labels: ['0'] },
    { tokens: 500_000, expected: 50, labels: ['500.0K', '500.00k', '50.00w'] },
    { tokens: 1_000_000, expected: 100, labels: ['1.0M', '1.00m', '100.00w'] },
    { tokens: 1_100_000, expected: 100, labels: ['1.1M', '1.10m', '110.00w'] }
  ])('Grok Free derives its 1M quota from local tokens: $tokens -> $expected%', async ({ tokens, expected, labels }) => {
    getUsage.mockResolvedValue({
    ...
    expect(wrapper.findAll('span').some((node) => labels.includes(node.text()))).toBe(true)
  ...
  it('Grok Free uses rolling 24h usage instead of today-only usage', async () => {
    ...
    expect(['750.0K', '750.00k', '75.00w'].some((label) => wrapper.text().includes(label))).toBe(true)
    expect(wrapper.text()).not.toContain('7d|')
    expect(wrapper.text()).not.toContain('200.0K')
    expect(wrapper.text()).not.toContain('250.0K')
    expect(wrapper.text()).not.toContain('20.00w')
    expect(wrapper.text()).not.toContain('25.00w')
  })
  ...
  it('Grok Free does not substitute today stats when rolling 24h usage is unavailable', async () => {
    ...
    expect(wrapper.text()).not.toContain('250.0K')
    expect(wrapper.text()).not.toContain('100.00w')
    expect(wrapper.text()).not.toContain('25.00w')
  })
  ...
  it('Grok paid manual probes keep the weekly/local summary when 24h usage is returned', async () => {
    ...
    expect(['1.0M', '100.00w'].some((label) => wrapper.text().includes(label))).toBe(true)
    expect(wrapper.text()).not.toContain('750.0K')
    expect(wrapper.text()).not.toContain('75.00w')
