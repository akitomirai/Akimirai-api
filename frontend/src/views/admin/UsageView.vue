<template>
  <AppLayout>
    <div class="space-y-6">
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <EntityDistributionChart
            v-model:metric="userDistributionMetric"
            :title="t('usage.userDistribution')"
            :entity-label="t('usage.user')"
      ...
      <div class="card">
        ...
        <div v-show="activeTab === 'diagnostics'" class="overflow-hidden rounded-b-2xl">
  ...
  <UsageDiagnosticsDrawer
    ...
    :usage-id="selectedDiagnosticsID"
    @openErrors="openRequestErrors"
  />
</template>
...
import UsageDiagnosticsDrawer from '@/components/admin/usage/UsageDiagnosticsDrawer.vue'
import RequestDiagnosticsTable from '@/components/admin/usage/RequestDiagnosticsTable.vue'
...
const userStats = ref<UserBreakdownItem[]>([])
const requestedModelStats = ref<ModelStat[]>([]); const upstreamModelStats = ref<ModelStat[]>([]); const mappingModelStats = ref<ModelStat[]>([]); const modelStatsLoading = ref(false)
const userStatsLoading = ref(false)
const userDistributionMetric = ref<DistributionMetric>('tokens')
...
const selectedDiagnosticsID = ref<number | null>(null)
...
const userDistributionItems = computed(() => userStats.value.map((item) => ({
  id: item.user_id,
  label: item.email || t('usage.userFallback', { id: item.user_id }),
...
const loadUserStats = async () => {
  ...
  try {
    const response = await adminAPI.dashboard.getUserBreakdown({
      ...
      stream: filters.value.stream ?? undefined,
      billing_type: filters.value.billing_type ?? undefined,
      limit: 200,
      sort_by: 'total_tokens',
    })
    if (seq !== userStatsReqSeq) return
    ...
    if (seq === userStatsReqSeq) userStatsLoading.value = false
  }
}
...
const openDiagnostics = (usageID: number) => {
  selectedDiagnosticsID.value = usageID
  diagnosticsVisible.value = true
}
const openRequestErrors = (requestID: string) => {
  filters.value = { ...filters.value, error_request_id: requestID }
...
// Column visibility
const ALWAYS_VISIBLE = ['user', 'created_at', 'diagnostics']
...
const allColumns = computed(() => [
  ...
  { key: 'diagnostics', label: t('admin.usage.diagnostics.title'), sortable: false, class: 'w-12' }
])
...
// Detail tabs
type DetailTab = 'usage' | 'errors' | 'ranking' | 'diagnostics'
...
const detailTabs = computed(() => [
  ...
  { key: 'diagnostics' as const, label: t('admin.usage.diagnostics.requestLogs'), icon: 'clipboard' as const },
])
...
const loadAdminErrors = async () => {
  ...
  try {
    const resp = await listErrorLogs({
      ...
      request_id: filters.value.error_request_id || undefined,
...
defineExpose({ requestedModelStats, userStats, refreshData })
</script>
