              ...
              <table class="w-full">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtToken(row.input_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtToken(row.output_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtToken(row.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtToken(row.cache_write_tokens) }}</td>
              ...
              <table class="w-full">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtToken(m.input_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtToken(m.output_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtToken(m.cache_creation_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtToken(m.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200">{{ fmtToken(m.total_tokens) }}</td>
...
const usageStatCells = computed<StatCell[]>(() => {
  ...
  return [
    ...
    { label: t('keyUsage.todayInputTokens'), value: fmtToken(today.input_tokens) },
    { label: t('keyUsage.todayOutputTokens'), value: fmtToken(today.output_tokens) },
    { label: t('keyUsage.todayTokens'), value: fmtToken(today.total_tokens) },
    { label: t('keyUsage.todayCacheCreation'), value: fmtToken(today.cache_creation_tokens) },
    { label: t('keyUsage.todayCacheRead'), value: fmtToken(today.cache_read_tokens) },
    { label: t('keyUsage.todayCost'), value: usd(today.actual_cost) },
    { label: t('keyUsage.rpmTpm'), value: `${fmtNum(usage.rpm || 0)} / ${fmtToken(usage.tpm || 0)}` },
    { label: t('keyUsage.totalRequests'), value: fmtNum(total.requests) },
    { label: t('keyUsage.totalInputTokens'), value: fmtToken(total.input_tokens) },
    { label: t('keyUsage.totalOutputTokens'), value: fmtToken(total.output_tokens) },
    { label: t('keyUsage.totalTokensLabel'), value: fmtToken(total.total_tokens) },
    { label: t('keyUsage.totalCacheCreation'), value: fmtToken(total.cache_creation_tokens) },
    { label: t('keyUsage.totalCacheRead'), value: fmtToken(total.cache_read_tokens) },
...
function fmtToken(val: number | null | undefined): string {
  if (val == null) return '-'
  return formatTokenCount(val, { locale: locale.value, display: 'exact' })
}
