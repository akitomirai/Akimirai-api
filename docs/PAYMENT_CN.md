...
| **个人收款码** | 支付宝、微信支付 | 使用个人支付宝/微信收款码，后台人工确认到账；不依赖安卓监听 App |
...
> 支付宝官方 / 微信官方、易支付、个人收款码可以同时作为后台服务商实例存在，但前台始终只展示 `支付宝`、`微信支付` 两个可见按钮。管理员需要分别为这两个按钮选择唯一支付来源：官方、易支付或个人收款码。官方渠道直接对接 API，资金直达商户账户，手续费更低；易支付通过第三方平台聚合，接入门槛更低；个人收款码适合小规模人工对账场景。
...
- **支付宝**：后台启用后，需要额外指定该按钮路由到 `支付宝官方`、`易支付支付宝` 或 `个人码支付宝`
- **微信支付**：后台启用后，需要额外指定该按钮路由到 `微信官方`、`易支付微信` 或 `个人码微信`
...
个人收款码服务商使用静态支付宝/微信收款码创建普通支付订单。第一版以后台人工确认为主：用户扫码付款后，管理员在支付宝/微信账单中核对金额和到账记录，再在后台订单中点击 **确认到账**。

| 参数 | 说明 | 必填 |
|------|------|------|
| **支付宝收款码内容** | 支付宝收款码解析内容、收款链接或图片 URL；启用支付宝时必填 | 条件必填 |
| **微信收款码内容** | 微信收款码解析内容、收款链接或图片 URL；启用微信时必填 | 条件必填 |
| **收款账号标识** | 对账用备注，例如账号昵称或收款人标识 | 否 |
| **通知密钥** | 预留给可信收款通知客户端的签名密钥；第一版可不填 | 否 |
| **匹配模式 / 金额窗口** | 预留给自动通知匹配；第一版人工确认不依赖它 | 否 |

> 静态个人收款码不能天然携带订单号，也不能通过 Sub2API 自动证明 payer 身份。若后续接入收款通知客户端，只能在签名、订单号、金额、支付方式和唯一待支付订单都匹配时自动确认；否则应继续人工复核。
...
> 个人收款码第一版不要求 Webhook。后台人工确认为主，签名通知接口为预留能力。
...
- 个人收款码订单不会自动查询上游状态；管理员需在订单超时宽限期内核对到账并人工确认
- 后台任务每 60 秒执行一次超时检查

### 退款与原路退款

- 官方支付宝、微信、Stripe、Airwallex、EasyPay 等支持网关退款的通道按各自 provider 能力处理。
- 个人收款码没有官方商户退款 API。管理员必须先在原支付宝/微信交易中完成原路退款，再在 Sub2API 后台填写退款凭据并确认退款。
- Sub2API 只记录并执行站内状态变更、余额扣回或订阅扣减；它不能代替支付宝/微信完成外部退款动作。
