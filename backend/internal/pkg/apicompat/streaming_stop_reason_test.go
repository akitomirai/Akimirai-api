...
func TestAnthropicStreamingMaxTokens_MapsToIncomplete(t *testing.T) {
	...
	AnthropicEventToResponsesEvents(&AnthropicStreamEvent{
		Type: "message_start", Message: &AnthropicResponse{ID: "msg_test", Model: "claude-opus-4-6", Role: "assistant"},
	}, state)
	AnthropicEventToResponsesEvents(&AnthropicStreamEvent{
		Type: "message_delta", Delta: &AnthropicDelta{StopReason: "max_tokens"}, Usage: &AnthropicUsage{OutputTokens: 4096},
	}, state)
...
func TestResponsesToChatCompletionsStreaming_ContentFilter(t *testing.T) {
	...
	events := ResponsesEventToChatChunks(&ResponsesStreamEvent{
		...
		Response: &ResponsesResponse{ID: "resp_cf", Status: "incomplete", IncompleteDetails: &ResponsesIncompleteDetails{Reason: "content_filter"}},
	}, state)
	...
	assert.True(t, hasContentFilter)
}
