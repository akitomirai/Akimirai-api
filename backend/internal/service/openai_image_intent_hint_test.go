...
func TestOpenAIGatewayServicePassthroughCompactModelMappingDoesNotChangeImageIntent(t *testing.T) {
	gin.SetMode(gin.TestMode)
	tests := []struct {
		name           string
		...
		{
			name:           "text to image mapping is ignored",
			canonicalModel: "gpt-5.4",
			compactModel:   "gpt-image-2",
		},
		{
			name:           "image to text mapping is ignored",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, tt.canonicalModel, gjson.GetBytes(upstream.lastBody, "model").String())
		})
	}
}
