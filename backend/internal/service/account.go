...
func (a *Account) GetGrokBaseURL() string {
	...
	baseURL := strings.TrimSpace(a.GetCredential("base_url"))
	if a.IsGrokOAuth() {
		// Legacy OAuth imports stored api.x.ai even though subscription text
		// traffic belongs on the CLI gateway. Regional endpoints and custom
		// relays remain explicit operator choices introduced in v0.1.162.
		if baseURL == "" || isOfficialGrokAPIBaseURL(baseURL) || !xai.IsParseableBaseURL(baseURL) {
