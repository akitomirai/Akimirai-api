	...
	for _, tt := range tests {
		t.Run(tt.model, func(t *testing.T) {
			pricingSvc := &PricingService{pricingData: map[string]*LiteLLMModelPricing{
				tt.model: {
					InputCostPerToken: tt.input, InputCostPerTokenPriority: tt.inputPriority,
					OutputCostPerToken: tt.output, OutputCostPerTokenPriority: tt.outputPriority,
					CacheReadInputTokenCost: tt.cacheRead, CacheReadInputTokenCostPriority: tt.cacheReadPriority,
				},
			}}
...
func TestParsePricingData_PreservesCapabilityMetadata(t *testing.T) {
	svc := &PricingService{}
	body := []byte(`{
		"gpt-5.6-sol": {
			"input_cost_per_token": 0.0000025,
	...
	metadata := svc.GetModelCapabilityMetadata("gpt-5.6-sol")
	require.NotNil(t, metadata)
	require.NotNil(t, metadata.MaxInputTokens)
	require.Equal(t, 1050000, *metadata.MaxInputTokens)
	require.NotNil(t, metadata.SupportsNativeStreaming)
	require.True(t, *metadata.SupportsNativeStreaming)
	require.NotNil(t, metadata.SupportsVision)
	require.True(t, *metadata.SupportsVision)
	require.NotNil(t, metadata.SupportsFunctionCalling)
	require.True(t, *metadata.SupportsFunctionCalling)
	require.NotNil(t, metadata.SupportsToolChoice)
	require.True(t, *metadata.SupportsToolChoice)
	require.NotNil(t, metadata.SupportsResponseSchema)
	require.True(t, *metadata.SupportsResponseSchema)
}
