...
func parseOllamaCloudUsageHTML(body []byte) (*OllamaCloudUsageData, error) {
	...
	if containsAnyOllamaUsage(lowerPage, "sign in to ollama", "log in to ollama", "continue to sign in") {
...
func ollamaModelWindow(node *html.Node) OllamaCloudUsageModelWindow {
	for block := parentNode(node); block != nil; block = block.Parent {
		if value, ok := htmlAttribute(block, "data-usage-window"); ok {
			...
			if containsAnyOllamaUsage(normalized, "five hour", "5 hour", "5h", "session") {
				return OllamaCloudUsageModelWindowFiveHour
			}
			if containsAnyOllamaUsage(normalized, "seven day", "7 day", "7d", "weekly") {
				return OllamaCloudUsageModelWindowSevenDay
			}
		}
		text := strings.ToLower(normalizedNodeText(block))
		fiveHour := containsAnyOllamaUsage(text, ollamaFiveHourUsageAliases...)
		sevenDay := containsAnyOllamaUsage(text, ollamaSevenDayUsageAliases...)
...
func containsAnyOllamaUsage(value string, candidates ...string) bool {
