...
func TestGPT56ExplicitZeroCacheWritePriceIsPreserved(t *testing.T) {
	...
	t.Run("flat channel price", func(t *testing.T) {
		resolved := &ResolvedPricing{
			Mode:        BillingModeToken,
			BasePricing: &ModelPricing{InputPricePerToken: 5e-6, OutputPricePerToken: 30e-6},
		}
		...
		cost, err := bs.CalculateCostUnified(CostInput{Model: "gpt-5.6-sol", Tokens: UsageTokens{CacheCreationTokens: 100}, RateMultiplier: 1, Resolver: resolver, Resolved: resolved})
	...
	t.Run("interval price", func(t *testing.T) {
		...
		cost, err := bs.CalculateCostUnified(CostInput{
			Model: "gpt-5.6-sol", Tokens: UsageTokens{CacheCreationTokens: 100}, RateMultiplier: 1,
			Resolver: resolver, Resolved: &ResolvedPricing{Mode: BillingModeToken, BasePricing: pricing},
		})
		require.NoError(t, err)
		require.Zero(t, cost.CacheCreationCost)
	})
