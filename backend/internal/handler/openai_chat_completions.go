...
func (h *OpenAIGatewayHandler) ChatCompletions(c *gin.Context) {
	...
	for {
		...
		authLatencyMs := getContextIntPtr(c, service.OpsAuthLatencyMsKey)
		routingLatencyMs := getContextIntPtr(c, service.OpsRoutingLatencyMsKey)
		...
		h.submitOpenAIUsageRecordTask(c.Request.Context(), result, func(ctx context.Context) {
			if err := h.gatewayService.RecordUsage(ctx, &service.OpenAIRecordUsageInput{
				...
				UpstreamLatencyMs:  upstreamLatencyMsPtr,
				ResponseLatencyMs:  responseLatencyMsPtr,
...
func resolveOpenAIUpstreamEndpoint(c *gin.Context, account *service.Account, result *service.OpenAIForwardResult) string {
	...
	if account != nil && account.Type == service.AccountTypeAPIKey && account.IsOpenAIPassthroughEnabled() {
		if _, ok := service.GetOpenAICompactBodySignal(c); ok {
			return "/v1/responses"
		}
	}
