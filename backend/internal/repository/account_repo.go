...
func (r *accountRepository) lockAndMergeAccountProbeExtra(ctx context.Context, client *dbent.Client, account *service.Account, explicitProbeEnabled *bool) (map[string]any, error) {
	...
	rows, err := client.QueryContext(ctx, `
		...
		FOR NO KEY UPDATE
	`, account.ID, account.Platform, account.Type, proxyID)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
	...
	if err := rows.Scan(&identityUnchanged, &storedCredentials, &currentEnabled, &currentSnapshot); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	...
	identityUnchanged = identityUnchanged && string(currentPayload) == string(incomingCredentials)

	return mergeAccountProbeExtra(account, explicitProbeEnabled, identityUnchanged, currentEnabled, currentSnapshot)
}
...
func mergeAccountProbeExtra(account *service.Account, explicitProbeEnabled *bool, identityUnchanged bool, currentEnabled, currentSnapshot []byte) (map[string]any, error) {
...
func (r *accountRepository) UpdateCredentials(ctx context.Context, id int64, credentials map[string]any) error {
	...
	if current.Platform == service.PlatformOpenAI &&
		...
		!bytes.Equal(currentPayload, incomingPayload) {
