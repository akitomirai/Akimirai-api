...
func NewAdminAccountRepository(client *dbent.Client, sqlDB *sql.DB, schedulerCache service.SchedulerCache, credentialEncryptor service.SecretEncryptor) service.AdminAccountRepository {
	return newAccountRepositoryWithSQL(client, sqlDB, schedulerCache, credentialEncryptor)
}
...
func (r *accountRepository) updateLockedAccount(ctx context.Context, client *dbent.Client, account *service.Account, explicitProbeEnabled *bool) (*dbent.Account, error) {
	extra, err := r.lockAndMergeAccountProbeExtra(ctx, client, account, explicitProbeEnabled)
	if err != nil {
		return nil, err
	}
	account.Extra = extra
...
func (r *accountRepository) lockAndMergeAccountProbeExtra(ctx context.Context, client *dbent.Client, account *service.Account, explicitProbeEnabled *bool) (map[string]any, error) {
	if r == nil || r.credentialEncryptor == nil {
		return lockAndMergeAccountProbeExtra(ctx, client, account, explicitProbeEnabled)
	}

	incomingCredentials, err := json.Marshal(normalizeJSONMap(account.Credentials))
	if err != nil {
		return nil, err
	}
	var proxyID any
	...
	rows, err := client.QueryContext(ctx, `
		SELECT
			platform,
			type,
			proxy_id IS NOT DISTINCT FROM $2,
			credentials,
	...
	if len(storedCredentials) > 0 && string(storedCredentials) != "null" {
		if err := json.Unmarshal(storedCredentials, &stored); err != nil {
			return nil, err
		}
	}
	currentCredentials, err := r.credentialsForService(normalizeJSONMap(stored))
	if err != nil {
		return nil, err
	}
	currentPayload, err := json.Marshal(normalizeJSONMap(currentCredentials))
	if err != nil {
		return nil, err
	}
	identityUnchanged := currentPlatform == account.Platform &&
		currentType == account.Type &&
		proxyIdentityUnchanged &&
		bytes.Equal(currentPayload, incomingCredentials)
	...
	currentAPIKey, err := json.Marshal(currentCredentials["api_key"])
	if err != nil {
		return nil, err
	}
	incomingAPIKey, err := json.Marshal(normalizeJSONMap(account.Credentials)["api_key"])
	if err != nil {
		return nil, err
	}
	ollamaGroupIdentityUnchanged := service.IsOllamaCloudUsageAccount(currentAccount) &&
		service.IsOllamaCloudUsageAccount(account) &&
		bytes.Equal(currentAPIKey, incomingAPIKey)

	return mergeAccountProbeExtra(
		account,
...
func (r *accountRepository) UpdateCredentials(ctx context.Context, id int64, credentials map[string]any) error {
	incomingCredentials := normalizeJSONMap(credentials)
	incomingPayload, err := json.Marshal(incomingCredentials)
	if err != nil {
		return err
	}
	...
	lockIfSupported := dbpredicate.Account(func(selector *entsql.Selector) {
		if selector.Dialect() != dialect.SQLite {
			selector.ForUpdate()
		}
	})
	current, err := client.Account.Query().
		Where(dbaccount.IDEQ(id), dbaccount.DeletedAtIsNil(), lockIfSupported).
		Only(ctx)
	if err != nil {
		if dbent.IsNotFound(err) {
	...
	currentCredentials, err := r.credentialsForService(normalizeJSONMap(current.Credentials))
	if err != nil {
		return err
	}
	currentPayload, err := json.Marshal(normalizeJSONMap(currentCredentials))
	...
	if !bytes.Equal(currentPayload, incomingPayload) {
		...
		incomingAccount := &service.Account{
			Platform:    current.Platform,
			Type:        current.Type,
		...
		incomingAPIKey, err := json.Marshal(incomingCredentials["api_key"])
		if err != nil {
			return err
		}
		ollamaGroupIdentityUnchanged := service.IsOllamaCloudUsageAccount(currentAccount) &&
			service.IsOllamaCloudUsageAccount(incomingAccount) &&
			bytes.Equal(currentAPIKey, incomingAPIKey)
...
func (r *accountRepository) ListSchedulableAIPlatformSources(ctx context.Context) ([]service.ConfiguredAIPlatformSource, error) {
	accounts, err := r.schedulableAccountsQuery(time.Now()).
		Select(dbaccount.FieldPlatform, dbaccount.FieldCredentials).
		All(ctx)
	if err != nil {
		return nil, err
	}

	sources := make([]service.ConfiguredAIPlatformSource, 0, len(accounts))
	for _, account := range accounts {
		// Model mappings are explicitly non-sensitive credential fields. Build the
		// projection from only those raw values so this public route never decrypts
		// API keys, sessions, cookies, or other account credentials.
		credentials := make(map[string]any, 2)
		if value, ok := account.Credentials["model_mapping"]; ok {
			credentials["model_mapping"] = value
		}
		if value, ok := account.Credentials["compact_model_mapping"]; ok {
			credentials["compact_model_mapping"] = value
		}
		projection := service.Account{
			Platform:    account.Platform,
			Credentials: credentials,
		}
		sources = append(sources, service.ConfiguredAIPlatformSource{
			Platform:            account.Platform,
			ModelMapping:        projection.GetModelMapping(),
			CompactModelMapping: projection.GetCompactModelMapping(),
		})
	}
	return sources, nil
}
