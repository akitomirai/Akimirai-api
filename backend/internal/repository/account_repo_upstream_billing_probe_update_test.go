...
func updatedAccountRows(id int64, extra string) *sqlmock.Rows {
	return updatedAccountRowsWithCredentials(id, `{"api_key":"sk-test"}`, extra)
}

func updatedAccountRowsWithCredentials(id int64, credentials, extra string) *sqlmock.Rows {
	now := time.Now()
	return sqlmock.NewRows(dbaccount.Columns).AddRow(
		id, now, now, nil, "test", nil, service.PlatformOpenAI, service.AccountTypeAPIKey,
		[]byte(credentials), []byte(extra), nil, nil, 1, nil, 1, 1.0,
