...
func TestUpdateCredentialsIdentityChangeClearsAllOllamaManagedExtra(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT .* FROM "accounts".*FOR UPDATE`).
		WillReturnRows(updatedAccountRowsWithCredentials(17,
			`{"api_key":"old-key","base_url":"https://ollama.com"}`,
			`{"ollama_cloud_usage_session":"cipher:session","ollama_cloud_usage_auto_refresh":true,"ollama_cloud_usage_snapshot":{"status":"ok"}}`))
	mock.ExpectExec(`(?s)UPDATE "accounts" SET .*credentials.*extra.*WHERE .*id.*`).
		WillReturnResult(sqlmock.NewResult(0, 1))
...
// Unchanged credentials must not rewrite extra, otherwise non-Ollama API-key
// accounts can lose unrelated managed snapshots during an idempotent save.
func TestUpdateCredentialsPreservesExtraWhenCredentialsAreUnchanged(t *testing.T) {
	client, mock := newOllamaCloudUsageRepositoryTestClient(t)
	mock.ExpectBegin()
	mock.ExpectQuery(`(?s)SELECT .* FROM "accounts".*FOR UPDATE`).
		WillReturnRows(updatedAccountRowsWithCredentials(17,
	...
	mock.ExpectExec(`(?s)UPDATE "accounts" SET .*credentials.*WHERE .*id.*`).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO scheduler_outbox")).
		WithArgs(service.SchedulerOutboxEventAccountChanged, int64(17), nil, nil, sqlmock.AnyArg()).
		WillReturnResult(sqlmock.NewResult(1, 1))
