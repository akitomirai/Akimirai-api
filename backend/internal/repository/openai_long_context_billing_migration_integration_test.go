...
func TestMigration181EnforcesOpenAILongContextBillingWriteInvariant(t *testing.T) {
	tx := testTx(t)
	ctx := context.Background()
	migrationSQL, err := dbmigrations.FS.ReadFile("181_default_openai_long_context_billing.sql")
...
VALUES ('migration-181-ordinary', 'openai', 'oauth', '{}'::jsonb)
RETURNING id
`).Scan(&ordinaryID))
	...
	require.NoError(t, tx.QueryRowContext(ctx, `
INSERT INTO accounts (name, platform, type, extra)
VALUES ('migration-181-parent', 'openai', 'oauth', '{"openai_long_context_billing_enabled":false}'::jsonb)
RETURNING id
`).Scan(&parentID))
...
VALUES ('migration-181-shadow', 'openai', 'oauth', '{}'::jsonb, $1, 'spark')
RETURNING id
`, parentID).Scan(&shadowID))
	...
	require.NoError(t, tx.QueryRowContext(ctx, `
INSERT INTO accounts (name, platform, type, extra)
VALUES ('migration-181-malformed-legacy', 'openai', 'oauth', '{"openai_long_context_billing_enabled":"false"}'::jsonb)
RETURNING id
...
`, malformedLegacyID).Scan(&malformedLegacyEnabled))
	...
	_, err = tx.ExecContext(ctx, `
UPDATE accounts
SET extra = extra || '{"migration_181_unrelated_update":true}'::jsonb
WHERE id = $1
`, malformedLegacyID)
	require.NoError(t, err)
...
`, shadowID).Scan(&preservedOptOutEvents))
	...
	require.NoError(t, tx.QueryRowContext(ctx, `
INSERT INTO accounts (name, platform, type, extra)
VALUES ('migration-181-rolling-writer', 'openai', 'oauth', '{}'::jsonb)
...
`, shadowID).Scan(&shadowOutboxEvents))
	...
	_, err = tx.ExecContext(ctx, `
INSERT INTO accounts (name, platform, type, extra)
VALUES ('migration-181-malformed', 'openai', 'oauth', '{"openai_long_context_billing_enabled":"false"}'::jsonb)
`)
